...
# --- Session State Management (For Resetting Uploader) ---
if 'uploader_key' not in st.session_state:
    st.session_state.uploader_key = 0

def reset_app():
    # Incrementing the key forces the file_uploader to reset completely
...
# --- UI LOGIC ---
# Shared key with session state to allow reset
active_file = None
...
if active_file is None:
    ...
    if active_file is None:
        st.markdown('<p style="text-align:center; color:#888;">Neural Enhancement tuned for your device.</p>', unsafe_allow_html=True)
...
if active_file:
    ...
    with col_b:
        ...
        _, buf = cv2.imencode('.jpg', output_bgr)
        st.download_button("📩 DOWNLOAD IMAGE", buf.tobytes(), "enhanced.jpg", "image/jpeg")
        
        # FIXED: THIS BUTTON NOW PROPERLY RESETS THE APP
        if st.button("➕ UPLOAD ANOTHER IMAGE"):
            reset_app()

st.divider()
st.caption("DeepSense AI | Responsive & Auto-Reset v7.0")
